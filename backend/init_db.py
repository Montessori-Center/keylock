...
def insert_test_data():
    """Вставка минимальных тестовых данных (только структура)"""
    try:
        connection = pymysql.connect(
            host=Config.DB_HOST,
            port=Config.DB_PORT,
        ...
        if count == 0:
            print("📝 Добавляю структуру данных...")
            
            # Вставляем только кампанию без конкретного названия
            cursor.execute("INSERT INTO campaigns (name) VALUES ('Основная кампания')")
            campaign_id = cursor.lastrowid
            connection.commit()
            
            # Вставляем группы объявлений без привязки к конкретной тематике
            ad_groups = [
                'Группа 01',
                'Группа 02', 
                'Группа 03',
            ...
            print("ℹ️  Тестовые ключевые слова не добавлены - БД готова к работе")
